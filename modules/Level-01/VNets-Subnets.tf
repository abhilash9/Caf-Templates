...
resource "azurerm_virtual_network" "virtualNetwork" {
    for_each                = local.vnet
    name                    = each.key
    resource_group_name     = azurerm_resource_group.resourceGroup[each.value.vnet_rg_name].name
    ...
    dynamic "subnet"{
        ...
        content{
            ...
            security_group  = subnet.value.sub_nsg_name != null ? azurerm_network_security_group.networkSecurityGroup[subnet.value.sub_nsg_name].id : null 
        }
    }
}
